...
async function ensureAthleteExists(athleteId, res) {
  const exists = await Athlete.exists({ _id: athleteId });
  if (!exists) {
    notFound(res, 'Athlete not found');
    return false;
  }
  return true;
}

function buildSessionsFilter(query, overrides = {}) {
  const filter = { ...overrides };
  if (query.athleteId && !filter.athleteId) filter.athleteId = query.athleteId;
  if (query.riskLevel) filter.riskLevel = query.riskLevel;
  if (query.weatherAlertLevel) filter.weatherAlertLevel = query.weatherAlertLevel;
...
async function paginateSessions(req, res, filter) {
...
async function enrichAndCreateSession(payload) {
...
app.get('/api/v1/athletes/:id', wrap(async (req, res) => {
  ...
  if (!item) return notFound(res, 'Athlete not found');
...
app.delete('/api/v1/athletes/:id', wrap(async (req, res) => {
  ...
  if (!deleted) return notFound(res, 'Athlete not found');
  res.status(204).send();
}));

app.get('/api/v1/athletes/:athleteId/sessions', wrap(async (req, res) => {
  if (!(await ensureAthleteExists(req.params.athleteId, res))) return;
  const filter = buildSessionsFilter(req.query, { athleteId: req.params.athleteId });
  await paginateSessions(req, res, filter);
}));

app.post('/api/v1/athletes/:athleteId/sessions', wrap(async (req, res) => {
  if (!(await ensureAthleteExists(req.params.athleteId, res))) return;
  const payload = await parseBody(req);
  const created = await enrichAndCreateSession({ ...payload, athleteId: req.params.athleteId });
  respond(req, res, 'TrainingSession', created.toObject(), 201);
}));

app.get('/api/v1/athletes/:athleteId/injuries', wrap(async (req, res) => {
  if (!(await ensureAthleteExists(req.params.athleteId, res))) return;
  const items = await Injury.find({ athleteId: req.params.athleteId }).sort({ injuryDate: -1 }).lean();
  respond(req, res, 'Injuries', { injury: items });
}));

app.post('/api/v1/athletes/:athleteId/injuries', wrap(async (req, res) => {
  if (!(await ensureAthleteExists(req.params.athleteId, res))) return;
  const payload = await parseBody(req);
  const created = await Injury.create({ ...payload, athleteId: req.params.athleteId });
  respond(req, res, 'Injury', created.toObject(), 201);
}));

app.post('/api/v1/sessions', wrap(async (req, res) => {
  const payload = await parseBody(req);
  const created = await enrichAndCreateSession(payload);
  respond(req, res, 'TrainingSession', created.toObject(), 201);
}));

app.get('/api/v1/sessions', wrap(async (req, res) => {
  const filter = buildSessionsFilter(req.query);
  await paginateSessions(req, res, filter);
}));

app.get('/api/v1/sessions/:id', wrap(async (req, res) => {
  const item = await TrainingSession.findById(req.params.id).lean();
  if (!item) return notFound(res, 'Session not found');
...
app.get('/api/v1/sessions/:sessionId/injuries', wrap(async (req, res) => {
  const sessionExists = await TrainingSession.exists({ _id: req.params.sessionId });
  if (!sessionExists) return notFound(res, 'Session not found');
  const items = await Injury.find({ sessionId: req.params.sessionId }).sort({ injuryDate: -1 }).lean();
  respond(req, res, 'Injuries', { injury: items });
}));
...
app.get('/api/v1/injuries', wrap(async (req, res) => {
  ...
  const items = await Injury.find(filter).sort({ injuryDate: -1 }).lean();
  respond(req, res, 'Injuries', { injury: items });
}));

app.get('/api/v1/injuries/:id', wrap(async (req, res) => {
  const item = await Injury.findById(req.params.id).lean();
  if (!item) return notFound(res, 'Injury not found');
...
app.delete('/api/v1/injuries/:id', wrap(async (req, res) => {
  ...
  if (!deleted) return notFound(res, 'Injury not found');
  res.status(204).send();
}));
